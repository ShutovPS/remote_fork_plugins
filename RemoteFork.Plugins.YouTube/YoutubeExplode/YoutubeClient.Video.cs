...
namespace YoutubeExplode
{
    public partial class YoutubeClient
    {
        ...
        private async Task<IReadOnlyDictionary<string, string>> GetVideoInfoDicAsync(string videoId, string sts = null)
        {
            ...
            if (result.GetValueOrDefault("video_id", string.Empty).IsNullOrWhiteSpace())
                throw new VideoUnavailableException(videoId, $"Video [{videoId}] is unavailable.");

            return result;
        }

        private async Task<string> GetVideoWatchPageHtmlAsync(string videoId)
        {
        ...
        private async Task<string> GetVideoEmbedPageHtmlAsync(string videoId)
        {
        ...
        private async Task<PlayerConfiguration> GetPlayerConfigurationAsync(string videoId)
        {
            // Try to get from video info
            {
                ...
                var playerConfigRaw = Regex.Match(videoEmbedPageHtml,
                ...
                if (errorReason.IsNullOrWhiteSpace())
                {
                    ...
                    var muxedStreamInfosUrlEncoded =
                        !isLiveStream ? videoInfoDic.GetValueOrDefault("url_encoded_fmt_stream_map", string.Empty) : null;
                    var adaptiveStreamInfosUrlEncoded =
                        !isLiveStream ? videoInfoDic.GetValueOrDefault("adaptive_fmts", string.Empty) : null;
                ...
                // If the video requires purchase - throw (approach two)
                {
                    ...
                    if (!previewVideoInfoRaw.IsNullOrWhiteSpace())
                    {
                        ...
                        var previewVideoId = previewVideoInfoDic.GetValueOrDefault("video_id", string.Empty);
            ...
            // Try to get from video watch page
            {
                ...
                var playerConfigRaw = Regex.Match(videoWatchPageHtml,
...
//                    throw new VideoUnplayableException(videoId, $"Video [{videoId}] is unplayable. Reason: {errorReason}");
        ...
        public async Task<Video> GetVideoAsync(string videoId)
        {
            ...
            DateTimeOffset videoUploadDate = DateTimeOffset.Now;
            var regex = new Regex("(<meta itemprop=\"datePublished\" content=\")([\\d\\-]+)(\")");
            if (regex.IsMatch(videoWatchPageHtml)) {
                videoUploadDate = regex.Match(videoWatchPageHtml).Groups[2].Value.ParseDateTimeOffset();
            }
            ...
            regex = new Regex("(button.like-button-renderer-like-button[\\s\\S]*?content\">)(.*?)(<)");
            if (regex.IsMatch(videoWatchPageHtml)) {
                var videoLikeCountRaw = regex.Match(videoWatchPageHtml).Groups[2].Value.StripNonDigit();
                videoLikeCount = !videoLikeCountRaw.IsNullOrWhiteSpace() ? videoLikeCountRaw.ParseLong() : 0;
            }
            ...
            //(button.like-button-renderer-dislike-button[\s\S]*?content">)(.*?)(<)
            long videoDislikeCount = 0;
            regex = new Regex("(button.like-button-renderer-dislike-button[\\s\\S]*?content\">)(.*?)(<)");

            if (regex.IsMatch(videoWatchPageHtml)) {
                var videoDislikeCountRaw = regex.Match(videoWatchPageHtml).Groups[2].Value.StripNonDigit();
                videoDislikeCount = !videoDislikeCountRaw.IsNullOrWhiteSpace() ? videoDislikeCountRaw.ParseLong() : 0;
            }
...
//            var videoDislikeCount = !videoDislikeCountRaw.IsNullOrWhiteSpace() ? videoDislikeCountRaw.ParseLong() : 0;
        ...
        public async Task<MediaStreamInfoSet> GetVideoMediaStreamInfosAsync(string videoId)
        {
            ...
            foreach (var streamInfoDic in muxedStreamInfoDics)
            {
                ...
                var signature = streamInfoDic.GetValueOrDefault("s", string.Empty);
                if (!signature.IsNullOrWhiteSpace())
                {
                    ...
                    var signatureParameter = streamInfoDic.GetValueOrDefault("sp", string.Empty) ?? "signature";
            ...
            foreach (var streamInfoDic in adaptiveStreamInfoDics)
            {
                ...
                // Try to extract content length, otherwise get it manually
                var contentLength = streamInfoDic.GetValueOrDefault("clen", string.Empty).ParseLongOrDefault();
                if (contentLength <= 0)
                {
                    // Send HEAD request and get content length
                    contentLength = await _httpClient.GetContentLengthAsync(url, false).ConfigureAwait(false) ?? 0;
