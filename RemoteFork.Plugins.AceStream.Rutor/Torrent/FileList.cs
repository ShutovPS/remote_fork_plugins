...
namespace RemoteFork.Plugins {
    public class FileList {
        public static Dictionary<string, string> GetFileList(byte[] torrentData, ref string id) {
            string contentId = HTTPUtility.PostRequest(PluginSettings.Settings.AceStreamApi.GetContentId,
                Encoding.UTF8.GetBytes(Convert.ToBase64String(torrentData)));
            ...
            if (regex.IsMatch(contentId)) {
                ...
                string aceMadiaInfo =
                    HTTPUtility.GetRequest(string.Format(PluginSettings.Settings.AceStreamApi.GetMediaFilesByTorrent,
                        Rutor.GetAddress, id));
        ...
        public static Dictionary<string, string> GetFileList(string pathTorrent) {
            string aceMadiaInfo =
                HTTPUtility.GetRequest(string.Format(PluginSettings.Settings.AceStreamApi.GetMediaFilesByMagnet,
                    Rutor.GetAddress, pathTorrent));
            if (!string.IsNullOrWhiteSpace(aceMadiaInfo)) {
                var magnet = JsonConvert.DeserializeObject<Magnet>(aceMadiaInfo);
                if (!string.IsNullOrEmpty(magnet.error)) {
                    throw new Exception(magnet.error);
